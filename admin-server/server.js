...
app.get("/config/music", rateLimit(30, 60000), function(req, res) {
  ...
  body += '<div class="form-section"><h3>音乐文件 (' + files.length + ' 个)</h3>';
  if (files.length > 0) {
    body += '<div style="display:flex;flex-direction:column;gap:6px">';
    files.forEach(function(f) {
      var fbase = f.name.replace(/\.[^.]+$/, '');
      var entry = cfg.music.find(function(m) { return m.url && m.url.indexOf(f.name) !== -1; }) || null;
      var dn = entry ? (entry.name || fbase) : fbase;
      var da = entry ? (entry.artist || '') : '';
      var cv = entry ? (entry.cover || '') : '';
      if (!cv) { var mf = coverFiles.find(function(cf) { return cf.name.replace(/\.[^.]+$/, '') === fbase; }); if (mf) cv = '/assets/music/cover/' + mf.name; }
      var ch = cv ? '<img src="' + esc(cv) + '" style="width:40px;height:40px;object-fit:cover;border-radius:6px;border:1px solid #e2e8f0;background:#fff;flex-shrink:0">' : '<div style="width:40px;height:40px;border-radius:6px;background:#f1f5f9;color:#94a3b8;display:flex;align-items:center;justify-content:center;font-size:16px;flex-shrink:0">&#127925;</div>';
      var hm = entry && (entry.name || entry.artist || entry.cover);
      var st = hm ? '<span style="font-size:11px;color:#16a34a">&#10003; 已配置</span>' : '<span style="font-size:11px;color:#f59e0b">未配置</span>';
      body += '<div style="display:flex;align-items:center;gap:10px;padding:8px 12px;background:#f8fafc;border-radius:6px">';
      body += '<div style="position:relative;flex-shrink:0">' + ch + '</div>';
      body += '<div style="flex:1;min-width:0"><div style="font-size:13px;font-weight:500;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + esc(dn) + '</div>';
      body += '<div style="font-size:12px;color:#64748b">' + esc(da || '未设置歌手') + ' &middot; ' + esc(f.name) + ' &middot; ' + f.size + ' ' + st + '</div></div>';
      body += '<div style="display:flex;gap:6px;flex-shrink:0">';
      body += '<button class="btn btn-primary btn-sm" onclick="editFileMusic(\'' + esc(f.name) + '\')">编辑</button>';
      body += '<button class="btn btn-danger btn-sm" onclick="deleteMusicFile(\'' + esc(f.name) + '\')">删除</button>';
      body += '</div></div>';
    });
    body += '</div>';
  } else {
    body += '<p style="color:#94a3b8;font-size:13px">暂无音乐文件</p>';
  }
  body += '</div></div>';
  ...
  body += '<div id="editFileName" style="font-size:12px;color:#94a3b8;margin-bottom:12px;word-break:break-all"></div>';
  ...
  body += 'var coverFilesData=' + JSON.stringify(coverFiles.map(function(cf){return{name:cf.name,path:'/assets/music/cover/'+cf.name}})) + ';';
  body += 'var editFile="";';
  ...
  body += 'function saveEditMusic(){if(!editFile)return;var name=document.getElementById("editMusicName").value.trim();if(!name){showToast("\u6b4c\u66f2\u540d\u4e0d\u80fd\u4e3a\u7a7a","error");return}var artist=document.getElementById("editMusicArtist").value.trim();var cover=document.getElementById("editMusicCoverUrl").value.trim();var url="/assets/music/"+editFile;var idx=musicData.findIndex(function(m){return m.url&&m.url.indexOf(editFile)!==-1});if(idx>=0){musicData[idx].name=name;musicData[idx].artist=artist;musicData[idx].cover=cover}else{musicData.push({name:name,artist:artist,url:url,cover:cover,lrc:""})}closeEditModal();saveMusicList()}';
  body += 'function publishConfig(){fetch("/api/config/publish",{method:"POST"}).then(function(r){return r.json()}).then(function(j){if(j.ok){showToast("\u2705 "+j.message,"success")}else{showToast("\u274c "+(j.error||"\u63a8\u9001\u5931\u8d25"),"error")}})}';
  body += '</script>';
  body += '</div></div>';
