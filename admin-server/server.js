...
app.get("/config/music", rateLimit(30, 60000), function(req, res) {
  ...
  body += 'var editIdx=-1;';
    body += 'function editMusic(i){editIdx=i;var m=musicData[i];document.getElementById("editMusicName").value=m.name||"";document.getElementById("editMusicArtist").value=m.artist||"";document.getElementById("editMusicCoverUrl").value=m.cover||"";var pv=document.getElementById("editMusicCoverPreview");pv.innerHTML="";if(m.cover){var img=document.createElement("img");img.src=m.cover;img.style.cssText="width:80px;height:80px;object-fit:cover;border-radius:8px";pv.appendChild(img)}document.getElementById("editCoverFileInput").value="";document.getElementById("editCoverList").innerHTML="";document.getElementById("editCoverLoading").style.display="block";document.getElementById("editMusicModal").style.display="flex";fetch("/api/config/music/covers").then(function(r){return r.json()}).then(function(j){document.getElementById("editCoverLoading").style.display="none";if(!j.ok||!j.covers.length)return;var list=document.getElementById("editCoverList");j.covers.forEach(function(c){var div=document.createElement("div");div.style.cssText="width:60px;height:60px;border-radius:6px;overflow:hidden;cursor:pointer;border:2px solid transparent;position:relative";div.title=c.name;div.onclick=function(){selectEditCover(c.path)};var ci=document.createElement("img");ci.src=c.path;ci.style.cssText="width:100%;height:100%;object-fit:cover";div.appendChild(ci);list.appendChild(div)})}).catch(function(){document.getElementById("editCoverLoading").style.display="none"})}';
  body += 'function closeEditModal(){document.getElementById("editMusicModal").style.display="none";editIdx=-1}';
  body += 'function selectEditCover(path){document.getElementById("editMusicCoverUrl").value=path;var pv=document.getElementById("editMusicCoverPreview");pv.innerHTML="";var img=document.createElement("img");img.src=path;img.style.cssText="width:80px;height:80px;object-fit:cover;border-radius:8px";pv.appendChild(img);var items=document.getElementById("editCoverList").children;for(var k=0;k<items.length;k++){var si=items[k].querySelector("img");items[k].style.borderColor=(si&&si.getAttribute("src")===path)?"#3b82f6":"transparent"}}';
